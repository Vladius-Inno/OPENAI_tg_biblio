...
1. Рекомендовать книги на основе ваших предпочтений - зайдите в Рекомендации через меню
2. Искать книги  - отправьте сообщение "Найди + ваш запрос"
2. Вместе с ChatGPT:
    - Писать и редактировать тексты
    - Переводить с и на разные языки
    - Обобщать информацию
    - Поддерживать беседу и запоминать контекст - просто отправьте мне сообщение.
...
🔄 Отправьте это сообщение другу.
    ...
    async def handle_recom_command(self, chat_id, markup):
        ...
        message = f"Поставьте оценки нескольким случайным книгам, и мы сможем начать подбирать для вас рекомендации. Найти нужную книгу можно отправив сообшение, начианающееся с 'найдиэ"
